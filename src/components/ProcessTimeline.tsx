        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="space-y-24">
            {processSteps.map((step, index) => (
              ...
              >
                {/* Content */}
                <div className={`flex-1 pl-20 lg:pl-0 ${index % 2 === 0 ? "lg:text-right" : "lg:text-left"}`}>
                  <motion.div
                    whileHover={{ scale: 1.02 }}
                    className="inline-block"
                  >
                    <div className="p-8 rounded-2xl bg-gradient-to-br from-muted/60 to-muted/20 border border-border/50 hover:border-primary/30 transition-all duration-300 group">
                      <span className={`inline-block text-6xl font-heading font-bold bg-gradient-to-r ${step.color} bg-clip-text text-transparent mb-4`}>
                        {step.number}
                      </span>
                ...
                {/* Node */}
                <div className="absolute left-8 lg:left-1/2 lg:-translate-x-1/2 flex items-center justify-center">
                  <motion.div
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    transition={{ delay: 0.2, type: "spring" }}
                    viewport={{ once: true }}
                    className={`w-4 h-4 rounded-full bg-gradient-to-r ${step.color} shadow-glow`}
                  />
                  <motion.div
                    className={`absolute w-8 h-8 rounded-full bg-gradient-to-r ${step.color} opacity-20`}
