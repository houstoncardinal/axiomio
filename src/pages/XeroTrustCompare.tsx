      ...
      <section className="py-20">
        <div className="container mx-auto px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto">
              Detailed comparison across security, deployment, and enterprise capabilities
            </p>
          </motion.div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-6 px-4 font-heading text-lg">Feature</th>
                  {competitors.map((comp) => (
                    <th 
                      key={comp.name} 
                      className={`text-center py-6 px-4 ${comp.highlight ? 'bg-primary/5' : ''}`}
                    >
                      <div className="flex flex-col items-center gap-1">
                        <span className={`font-heading text-lg ${comp.highlight ? 'text-primary' : 'text-foreground'}`}>
                          {comp.name}
                        </span>
                        <span className="text-xs text-muted-foreground">{comp.tagline}</span>
                      </div>
                    </th>
                  ))}
                </tr>
                ...
                {comparisonFeatures.map((category) => (
                  <React.Fragment key={category.category}>
                    <tr className="bg-muted/30">
                      <td colSpan={5} className="py-4 px-4 font-semibold text-foreground uppercase tracking-wide text-sm">
                        {category.category}
                      </td>
                    </tr>
                    {category.features.map((feature, index) => (
                      <tr key={feature.name} className={`border-b border-border/50 ${index % 2 === 0 ? '' : 'bg-muted/5'}`}>
                        <td className="py-4 px-4 text-foreground">{feature.name}</td>
                        <td className="py-4 px-4 text-center bg-primary/5">
                          <div className="flex justify-center">
                            <FeatureValue value={feature.xeroTrust} />
                          </div>
                        </td>
                        <td className="py-4 px-4 text-center">
                          <div className="flex justify-center">
                            <FeatureValue value={feature.cloudflare} />
                          </div>
                        </td>
                        <td className="py-4 px-4 text-center">
                          <div className="flex justify-center">
                            <FeatureValue value={feature.zscaler} />
                          </div>
                        </td>
                        <td className="py-4 px-4 text-center">
                          <div className="flex justify-center">
                            <FeatureValue value={feature.tailscale} />
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          <div className="lg:hidden space-y-8">
            {comparisonFeatures.map((category) => (
              <div key={category.category}>
                <h3 className="font-heading text-xl font-semibold text-foreground mb-4 border-b border-border pb-2">
                  {category.category}
                </h3>
                <div className="space-y-4">
                  {category.features.map((feature) => (
                    <div key={feature.name} className="p-4 rounded-xl bg-card border border-border">
                      <p className="font-medium text-foreground mb-3">{feature.name}</p>
                      <div className="grid grid-cols-4 gap-2 text-center text-sm">
                        <div className="p-2 rounded bg-primary/10">
                          <p className="text-xs text-muted-foreground mb-1">XeroTrust</p>
                          <div className="flex justify-center">
                            <FeatureValue value={feature.xeroTrust} />
                          </div>
                        </div>
                        <div className="p-2 rounded bg-muted/30">
                          <p className="text-xs text-muted-foreground mb-1">Cloudflare</p>
                          <div className="flex justify-center">
                            <FeatureValue value={feature.cloudflare} />
                          </div>
                        </div>
                        <div className="p-2 rounded bg-muted/30">
                          <p className="text-xs text-muted-foreground mb-1">Zscaler</p>
                          <div className="flex justify-center">
                            <FeatureValue value={feature.zscaler} />
                          </div>
                        </div>
                        <div className="p-2 rounded bg-muted/30">
                          <p className="text-xs text-muted-foreground mb-1">Tailscale</p>
                          <div className="flex justify-center">
                            <FeatureValue value={feature.tailscale} />
                          </div>
                        </div>
